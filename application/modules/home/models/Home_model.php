...
class Home_model extends CI_Model {
    ...
    var $column_search = array('jobs.title', 'jobs.description'); //set column field database for datatable searchable 
    ...
    public function getJobs($id){
        return $this->db->select('jobs.*, category.name')->join('category', 'category.id=jobs.type')->where('jobs.id', base64_decode($id))->get('jobs')->row_array();
    }
    ...
    public function jobList($params=[]){
        $this->db->select('jobs.*, category.name')->join('category', 'category.id=jobs.type')->where('jobs.status', 0);
        if(count($params)>=1){
            if($params['type']=='view'){
                $this->db->limit(15, $params['id']);
            }else if($params['type']=='location' && $params['selectedval']!=''){
                $this->db->where('city', $params['selectedval'])->limit(15, 0);
            }else if($params['type']=='category' && $params['selectedval']!=''){
                $this->db->where('type', $params['selectedval'])->limit(15, 0);
            }else if($params['type']=='experience' && $params['selectedval']!=''){
                $exp = explode('_', $params['selectedval']);
                if(isset($exp[1])){
                    $this->db->where('start_experience>', $exp[0]);
                    $this->db->where('start_experience<', $exp[0]);
                }else{
                    $this->db->where('end_experience>', $exp[0]);
                }
                $this->db->limit(15, 0);
            }else{
                $this->db->limit(15, 0);
    ...
    public function getLocations(){
        return $this->db->select('city')->group_by('city')->get('jobs')->result_array();
    }
}